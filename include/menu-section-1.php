<!-- list menu section - 1 -->

    <li id='<?php $id_page = "00"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Introduction</a></li>
    <li id='<?php $id_page = "01"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 1</a></li>
    <li id='<?php $id_page = "02"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 2</a></li>
    <li id='<?php $id_page = "03"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 3</a></li>
    <li id='<?php $id_page = "04"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 4</a></li>
    <li id='<?php $id_page = "05"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 5</a></li>
    <li id='<?php $id_page = "06"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 6</a></li>
    <li id='<?php $id_page = "07"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 7</a></li>
    <li id='<?php $id_page = "08"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 8</a></li>
    <li id='<?php $id_page = "09"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 9</a></li>
    <li id='<?php $id_page = "10"; ?>' class=""><a href="documentation.php?id='<?php echo $id_page; ?>' ">Documentation 10</a></li>